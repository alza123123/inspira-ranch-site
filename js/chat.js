(function () {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      #alza-chat-btn {
        ...
        opacity: 0.92;
        transition: background 0.2s, transform 0.2s, opacity 0.2s, bottom 0.3s;
        outline: none;
      }
      #alza-chat-btn:hover, #alza-chat-btn:focus-visible { background: ${COLORS.navyLight}; transform: scale(1.08); opacity: 1; }
      #alza-chat-btn:focus-visible { outline: 2px solid ${COLORS.gold}; outline-offset: 3px; }
      #alza-chat-btn svg { width: 22px; height: 22px; }
      /* lift the launcher clear of the cookie banner while it is on screen */
      body:has(#cookie-banner.visible) #alza-chat-btn { bottom: 130px; }
      ...
      @media (max-width: 420px) {
        ...
        #alza-chat-btn { bottom: 12px; right: 12px; width: 44px; height: 44px; }
      }
    `;
  ...
  function buildUI() {
    ...
    panel.innerHTML = `
      ...
      <div id="alza-messages" data-lenis-prevent>
  ...
  function togglePanel(open) {
    ...
    if (isOpen) {
      ...
      // Hide the launcher while the panel is open so it can never cover the
      // input/Send controls (mobile overlay bug fix). Close via X or Escape.
